...
dotenv.config();
const hsClient = new HyperspaceClient("eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJSaWtpIiwibmFtZSI6Ikh5cGVyc3BhY2UiLCJpYXQiOjE1MTYyMzkwMjJ9.iLr-HBp12Scf4K3pkJJ38wo036ecz6F3aASyGq34BtM");
const main = async () => {
    ...
    for (const collection of nftCollections) {
        fetchFunctions.push(new Promise(async (resolve, reject) => {
    ...
    const listings = listingsResponses.flat(1);
    // Sort from earliest block timestamp to latest
    const sortedListings = sortArrayByBlockTimestamp(listings);
    // Check each listing
    sortedListings.forEach((listing) => lib.checkListings(hsClient, listing));
};
console.log("Online");
